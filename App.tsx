...
import { GeminiLiveService, drawShapeTool, addTextTool, removeElementTool, clearBoardTool, exportCanvasTool, SYSTEM_INSTRUCTION } from './services/geminiLive';
...
const App: React.FC = () => {
  ...
  const handleTool = (name: string, args: any) => {
    ...
    // Handle export specially since it uses canvasRef
    if (name === 'export_canvas') {
      const format = args.format as 'svg' | 'png' | 'jpg';
      const filename = args.filename;
      canvasRef.current?.exportAs(format, filename);
      return;
    }
      ...
      try {
        ...
        while (response.functionCalls && response.functionCalls.length > 0 && iterations < MAX_ITERATIONS) {
          ...
          for (const fc of response.functionCalls) {
            // Handle export specially - execute immediately on main canvas
            if (fc.name === 'export_canvas') {
              const format = fc.args.format as 'svg' | 'png' | 'jpg';
              const filename = fc.args.filename;
              // Use the main canvas ref for export
              canvasRef.current?.exportAs(format, filename);
            } else {
      ...
      <header className="h-16 border-b border-slate-700 flex items-center justify-between px-6 bg-slate-800/50 backdrop-blur-md z-10 shrink-0">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button
            ...
            className={`flex items-center space-x-2 px-4 py-2 rounded-full font-medium transition-all ${status === ConnectionStatus.CONNECTED
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {processingPhase !== 'idle' && (
                <div className="flex items-center space-x-3 px-3 py-2 bg-slate-700/30 rounded-lg border border-slate-600/50 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className={`p-1.5 rounded-full flex items-center justify-center shadow-lg shadow-black/20 ${processingPhase === 'fixing' ? 'bg-orange-500/20 text-orange-400 ring-1 ring-orange-500/40' :
